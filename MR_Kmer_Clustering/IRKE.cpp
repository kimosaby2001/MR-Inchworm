...
unsigned int IRKE::compute_sequence_assemblies(unsigned long long zone, ofstream &outFile, float min_connectivity, unsigned int MIN_ASSEMBLY_LENGTH, unsigned int MIN_ASSEMBLY_COVERAGE, 
   				       bool WRITE_COVERAGE, string COVERAGE_OUTPUT_FILENAME) {

	// use self kcounter
	unsigned int num_iworm = compute_sequence_assemblies(zone, outFile, kcounter, min_connectivity, MIN_ASSEMBLY_LENGTH, MIN_ASSEMBLY_COVERAGE, 
				    WRITE_COVERAGE, COVERAGE_OUTPUT_FILENAME);
...
unsigned int IRKE::compute_sequence_assemblies(unsigned long long zone, ofstream &outFile, KmerCounter& kcounter, float min_connectivity,
...
//		cerr << "round: " << i << endl;
       ...
       // 	cout << "SEED kmer: " << kcounter.get_kmer_string(kmer) << ", count: " << kmer_count << endl;
		...
		if (sequence.length() >= MIN_ASSEMBLY_LENGTH && avg_cov >= MIN_ASSEMBLY_COVERAGE) {
			...
			headerstream << ">a" << INCHWORM_ASSEMBLY_COUNTER << "_" << zone  << ";" << avg_cov 
                         	     << " total_counts: " <<  total_counts << " Fpath: " << selected_path_n_pair_forward.second 
			             << " Rpath: " << selected_path_n_pair_reverse.second << " Seed: " << kmer_count 
				     << " K: " << kmer_length << " length: " << sequence.length();
		...
		//	cout << header << endl << sequence << endl;
			outFile << header << endl << sequence << endl;
	...
	return(INCHWORM_ASSEMBLY_COUNTER); // end of runIRKE
}
